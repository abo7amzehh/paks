...
function saf()
  ...
  function xgxc(szpy, qmxg)
    do
      do
        for _FORV1_5_ = 1, #qmxg do
          xgpy = szpy + qmxg[_FORV1_5_].offset
          xglx = qmxg[_FORV1_5_].type
          xgsz = qmxg[_FORV1_5_].value
          gg.setValues({
            [1] = {
              address = xgpy,
              flags = xglx,
  ...
  function xqmnb(qmnb)
    ...
    if gg.getResultCount() == 0 then
      gg.toast(qmnb[2].name .. "   Magician Boy")
    else
      ...
      if gg.getResultCount() == 0 then
        ...
        do
          do
            for _FORV1_4_ = 1, sz do
              pdsz = true
              do
                do
                  for _FORV1_8_ = 4, #qmnb do
                    if pdsz == true then
                      pysz = {}
                      pysz[1] = {}
                      pysz[1].address = sl[_FORV1_4_].address + qmnb[_FORV1_8_].offset
                      pysz[1].flags = qmnb[_FORV1_8_].type
                      szpy = gg.getValues(pysz)
                      pdpd = qmnb[_FORV1_8_].lv .. ";" .. szpy[1].value
                      szpd = split(pdpd, ";")
                      tzszpd = szpd[1]
                      pyszpd = szpd[2]
  ...
  if gg.getResultCount() == 0 then
    gg.alert(" لم يتم التفعيل\n اغلق اللعبة وقم بالمحاولة مرة أخرى ❌")
    os.exit()
  else
    ...
    do
      do
        for _FORV1_3_ = 1, resultsCount do
          gg.addListItems({
            [1] = {
              address = results[_FORV1_3_].address + 48,
              flags = 4,
              freeze = true,
              value = 119
            }
          ...
          gg.addListItems({
            [1] = {
              address = results[_FORV1_3_].address + 100,
              flags = 4,
              freeze = true,
              value = 70086
            }
   ...
   if (count > 0) then
        ...
        if (#data > 0) then
           ...
           gg.addListItems(t)
            ...
            gg.toast(" فشل ايجاد البيانات اعد التشغيل", false)
            return false
        end
    else
        gg.alert("Not Found")
